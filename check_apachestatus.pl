...
my $o_url        = '/server-status';
my $o_ssl        = undef;  # use https proto
...
sub print_usage {
  print "Usage: $Name -H <host> [--ssl] [-p <port>] [-t <timeout>] [-u <url>] [-w <warn_level> -c <crit_level>] [-V]\n";
}
...
-p, --port=PORT
   Http port
-u, --url=URL
   Status URL (/server-status is default)
-t, --timeout=INTEGER
...
my $response = undef;
my $schema = $o_ssl ? 'https://' : 'http://';

if (!defined($o_port)) {
  $response = $ua->get($schema . $o_host . $o_url);
} else {
  $response = $ua->get($schema . $o_host . ':' . $o_port . $o_url);
}
